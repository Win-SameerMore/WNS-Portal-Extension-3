report 50007 "Goods Receive Note SSE"
{
    DefaultLayout = RDLC;
    RDLCLayout = './src/Reports/Layout/50007.GoodsReceiveNote.rdl';
    Caption = 'Goods Received Note SSE';
    ...
    dataset
    {
        dataitem("Purch. Rcpt. Header"; "Purch. Rcpt. Header")
        {
            ...
            RequestFilterHeading = 'Goods Received Note';
            ...
            column(VendCountry; VendCountry)
            {
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    dataitem("Purch. Rcpt. Line"; "Purch. Rcpt. Line")
                    {
                        ...
                        column(UOMCode_PurchRcptLine; "Unit of Measure Code")
                        {
                        }
                        ...
                        begin
                            //WIN504>>
                            if "Purch. Rcpt. Line"."No." <> xItemNo then
                                intSrNo += 1;
                            xItemNo := "Purch. Rcpt. Line"."No.";

                            rILE.Reset();
                            rILE.SetRange("Document No.", "Purch. Rcpt. Line"."Document No.");
                            rILE.SetRange("Document Line No.", "Purch. Rcpt. Line"."Line No.");
                            if rILE.FindSet() then
                                repeat
                                    if txtsnno <> '' then
                                        txtSnNo += ',' + rILE."Serial No."
                                    else
                                        txtSnNo := 'Serial Number: ' + rILE."Serial No.";
                                until rile.next = 0;
                            //WIN504<<
            ...
            begin
                Countries.Reset();
                if Countries.get("Buy-from Country/Region Code") then
                    VendCountry := Countries.Name;
    ...
    var
        ...
        txtSnNo: text;
        DocumentTitle_Lbl: Label 'Goods Received Note';
    ...
    begin
        ...
        //Buy From Address
        Country.Reset();
        Country.SetRange(Code, PurchRcptHeader."Buy-from Country/Region Code");
        if Country.FindFirst() then;

        contact.Reset();
        contact.SetRange("No.", PurchRcptHeader."Buy-from Contact No.");
        if contact.FindFirst() then;

        VendAddr[1] := PurchRcptHeader."Buy-from Vendor Name";
        VendAddr[2] := PurchRcptHeader."Buy-from Address";
        VendAddr[3] := PurchRcptHeader."Buy-from Address 2";
        VendAddr[4] := Country.Name + ' ' + PurchRcptHeader."Buy-from Post Code";
        VendAddr[5] := contact."Phone No.";
        VendAddr[6] := contact.Name;

        //Ship to address
        Country.Reset();
        Country.SetRange(Code, PurchRcptHeader."Ship-to Country/Region Code");
        if Country.FindFirst() then;

        ShiptoAddr[1] := PurchRcptHeader."Ship-to Name";
        ShiptoAddr[2] := PurchRcptHeader."Ship-to Address";
        ShiptoAddr[3] := PurchRcptHeader."Ship-to Address 2";
        ShiptoAddr[4] := Country.Name + ' ' + PurchRcptHeader."Ship-to Post Code";
        ShiptoAddr[5] := '';
        ShiptoAddr[6] := PurchRcptHeader."Ship-to Contact";
