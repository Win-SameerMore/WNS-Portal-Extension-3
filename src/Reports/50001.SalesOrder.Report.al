report 50001 "Sales - Order SSE"
{
    RDLCLayout = './src/Reports/Layout/50001.SalesOrder.rdl';
    Caption = 'Sales - Order SSE';
    DefaultLayout = RDLC;
    ...
    dataset
    {
        dataitem(Header; "Sales Header")
        {
            ...
            column(Invoice_Lbl; SalesOrderLbl)
            {
            }
            ...
            column(txtTotalVATAmountText; TotalVATAmountText)
            {
            ...
            dataitem(Line; "Sales Line")
            {
                ...
                column(txtSerialNo; txtSerialNo)
                {
                ...
                begin
                    ...
                    countries.Reset();
                    if Countries.get(Header."Bill-to Country/Region Code") then//WIN504
                        BillToCountry := countries.Name;

                    countries.Reset();
                    if Countries.get(Header."Ship-to Country/Region Code") then//WIN504
                        ShipToCountry := countries.Name;

                    //WIN504 SRNO>>
                    if (Line."No." <> '') AND (Line.Type <> line.Type::" ") then
                        intSrNo += 1;
                    xItemNo := Line."No.";
                    //WIN504 SRNO<<
                    txtSerialNo := '';
                    rReservationEntry.reset();
                    rReservationEntry.SetRange("Source Type", 37);
                    rReservationEntry.SetRange("Source ID", Line."Document No.");
                    rReservationEntry.SetRange("Source Ref. No.", Line."Line No.");
                    if rReservationEntry.FindFirst() then
                        repeat
                            if txtSerialNo = '' then
                                txtSerialNo := rReservationEntry."Serial No."
                            else
                                txtSerialNo += ',' + rReservationEntry."Serial No.";
            ...
            begin
                ...
                if "Amount Including VAT" - Amount <> 0 then
                    VATPer := ("Amount Including VAT" - Amount) / "Amount Including VAT" * 100;
                ...
                CustAddr[4] := BillToCountry + ' ' + "Bill-to Post Code";
                ...
                ShipToAddr[2] := "Ship-to Address";
                ShipToAddr[3] := "Ship-to Address 2";
                ShipToAddr[4] := ShipToCountry + ' ' + "Ship-to Post Code";
                ShipToAddr[5] := '';
                ShipToAddr[6] := "Ship-to Contact";
