report 50006 "Purchase - Order SSE"
{
    RDLCLayout = './src/Reports/Layout/50006.PurchaseOrder.rdl';
    Caption = 'Purchase - Order SSE';
    DefaultLayout = RDLC;
    ...
    dataset
    {
        dataitem("Purchase Header"; "Purchase Header")
        {
            ...
            column(PurchTerm1; PurchSetup."Purchase Order Terms 1")
            {
            }
            ...
            column(PurchTerm2; PurchSetup."Purchase Order Terms 2")
            {

            }
            column(PurchTerm3; PurchSetup."Purchase Order Terms 3")
            {

            }
            column(decTotalSubTotal; Amount)
            {

            }
            column(decTotalVATAmount; "Amount Including VAT" - Amount)
            {
            ...
            column(decTotalAmountIncVAT; "Amount Including VAT")
            {
            }
            column(txtVATText; txtVATText)
            {
            }
            ...
            dataitem("Purchase Line"; "Purchase Line")
            {
                ...
                column(RequestedReceiptDate; "Expected Receipt Date")
                {
                }
                ...
                begin
                    //WIN504>>
                    if ("Purchase Line"."No." <> xItemNo) AND ("Purchase Line".Type <> "Purchase Line".Type::" ") then
                        IntSrNo += 1;
                    xitemno := "Purchase Line"."No.";
                    ///WIN504<<
            ...
            begin
                ...
                rPurchLine.Reset();
                rPurchLine.SetRange("Document No.", "No.");
                if rPurchLine.FindSet() then
                    repeat
                        decTotalSubTotal += rPurchLine."Line Amount";
                        decTotalVATAmount += rPurchLine."VAT Amount (ACY)";
                        decTotalAmountIncVAT += rPurchLine.Amount + rPurchLine."VAT Amount (ACY)";

                    until rPurchLine.Next = 0;

                txtVATText := TempVATAmountLine.VATAmountText;
                ...
                VATAmount1 := "Purchase Header"."Amount Including VAT" - "Purchase Header".Amount;
                if VATAmount1 <> 0 then
                    VATPer :=
                      Round((VATAmount1 / "Purchase Header"."Amount Including VAT") * 100, 1)
                else
                    VATPer := 0;
            end;
        }
    ...
    var
        ...
        IntSrNo: Integer;
        xItemNo: Text;
        decTotalSubTotal: Decimal;
        decTotalVATAmount: Decimal;
        decTotalAmountIncVAT: Decimal;
        txtVATText: Text;
    ...
    begin
        ...
        */
        //Buy From Address
        Country.Reset();
        Country.SetRange(Code, PurchaseHeader."Buy-from Country/Region Code");
        if Country.FindFirst() then;

        contact.Reset();
        contact.SetRange("No.", "Purchase Header"."Buy-from Contact No.");
        ...
        BuyFromAddr[4] := Country.Name + ' ' + PurchaseHeader."Buy-from Post Code";
        //CompressArray(BuyFromAddr);

        BuyFromAddr[5] := contact."Phone No.";
        BuyFromAddr[6] := contact.Name;

        //Ship-to Address
        FormatAddr.PurchHeaderShipTo(ShipToAddr, PurchaseHeader);
        Country.Reset();
        Country.SetRange(Code, PurchaseHeader."Ship-to Country/Region Code");
        if Country.FindFirst() then;
        ...
        ShiptoAddr[4] := Country.Name + ' ' + PurchaseHeader."Ship-to Post Code";
        //CompressArray(ShiptoAddr);

        ShiptoAddr[5] := purchaseHeader."Ship-To Phone";
        ShiptoAddr[6] := PurchaseHeader."Ship-to Contact";
