report 50002 "Proforma Invoice - SSE"
{
    RDLCLayout = './src/Reports/Layout/50002.ProformaInvoice.rdl';
    Caption = 'Proforma Invoice - SSE';
    DefaultLayout = RDLC;
    ...
    dataset
    {
        dataitem(Header; "Sales Header")
        {
            ...
            column(ProjectName; header."Project Name") { }
            column(CUrrencyCode; header."Currency Code") { }
            column(SOTerms1; SalesSetup."Sales Order Terms 1") { }
            column(SOTerms2; SalesSetup."Sales Order Terms 2") { }
            column(CompCertificateNo; CompanyInfo."Certificate No.") { }
            column(CompISONo; CompanyInfo."ISO No.") { }
            column(CompPic2; CompanyInfo."Picture 2") { }
            column(CompPic3; CompanyInfo."Picture 3") { }
            column(CompRegistrationNo; CompanyInfo."Registration No.") { }
            column(ReqDeliveryDate; header."Requested Delivery Date") { }
            column(Currency_Code; CurrCode) { }
            column(Currency_Code_LCY; LCYCurrCode) { }
            column(CompanyAddress1; CompanyAddr[1]) { }
            column(CompanyAddress2; CompanyAddr[2]) { }
            column(CompanyAddress3; CompanyAddr[3]) { }
            column(CompanyAddress4; CompanyAddr[4]) { }
            column(CompanyAddress5; CompanyAddr[5]) { }
            column(CompanyAddress6; CompanyAddr[6]) { }            //WIN504>>
            ...
            column(CompBankDetail1; CompanyInfo."Bank Detail 1")
            {

            }
            column(CompBankDetail2; CompanyInfo."Bank Detail 2")
            {

            }
            column(CompBankDetail3; CompanyInfo."Bank Detail 3")
            {

            }
            column(CompBankDetail4; CompanyInfo."Bank Detail 4")
            {

            }
            column(CompQRCode; CompanyInfo."QR Code")
            {
            ...
            column(Invoice_Lbl; ProformaLbl)
            {
            }
            ...
            column(TotalOrdeAmount; TotalOrdeAmount)
            {
            }
            column(txtTotalVATAmountText; VATPer)
            {
            }
            dataitem(Line; "Sales Line")
            {
                ...
                begin
                    ...
                    countries.Reset();
                    Countries.setrange(Code, Header."Bill-to Country/Region Code");//WIN504
                    if countries.FindFirst() then
                        BillToCountry := countries.Name;

                    countries.Reset();
                    Countries.setrange(Code, Header."Ship-to Country/Region Code");//WIN504
                    if countries.FindFirst() then
                        ShipToCountry := countries.Name;
                    //WIN504 SRNO>>
                    Hidevalue := false;
                    if (Line."No." <> '') AND (Line.Type <> line.Type::" ") then begin
                        intSrNo += 1;
                        Hidevalue := True;
                    end;
                    xItemNo := Line."No."
            ...
            begin
                ...
                if Header."Currency Code" = '' then begin
                    CurrCode := GenLedgSetup."LCY Code";
                end else begin
                    CurrCode := header."Currency Code";
                end;
                LCYCurrCode := GenLedgSetup."LCY Code";

                //ReportTotalsLine.Add(VATAmountLine.VATAmountText, TotalAmountVAT, false, true, false);
                rSalesLine.reset;
                rSalesLine.SetRange("Document No.", Header."No.");
                if rSalesLine.FindSet() then
                    repeat
                        TotalSubAmount += rSalesLine.Amount;
                        TotalTaxAmount += rSalesLine."Amount Including VAT" - rsalesline.Amount;
                        TotalOrdeAmount += rSalesLine."Amount Including VAT";

                    until rSalesLine.Next = 0;

                CompanyInfo.CalcFields("Picture 2");
                CompanyInfo.CalcFields("Picture 3");
                CompanyInfo.CalcFields("QR Code");
                FirstLineHasBeenOutput := false;
                Clear(Line);
                Clear(SalesPost);
                ...
                /* if VATAmountLine."VAT Amount" <> 0 then
                    VATPer := VATAmountLine."VAT %"
                else
                    VATPer := 0; */

                if "Amount Including VAT" - Amount <> 0 then
                    VATPer := Round(("Amount Including VAT" - Amount) / "Amount Including VAT" * 100);
                ...
                CustAddr[1] := Header."Bill-to Name";
                CustAddr[2] := Header."Bill-to Address";
                CustAddr[3] := Header."Bill-to Address 2";
                CustAddr[4] := BillToCountry + ' ' + Header."Bill-to Post Code";
                CustAddr[5] := BillToContact."Phone No.";
                CustAddr[6] := Header."Bill-to Contact";

                ShipToAddr[1] := Header."Ship-to Name";
                ShipToAddr[2] := Header."Ship-to Address";
                ShipToAddr[3] := Header."Ship-to Address 2";
                ShipToAddr[4] := ShipToCountry + ' ' + Header."Ship-to Post Code";
                ShipToAddr[5] := '';
                ShipToAddr[6] := Header."Ship-to Contact";
                ...
                if "Currency Code" <> '' then begin
                    ...
                    CalculatedExchRate :=
                      (1 / "Currency Factor" * CurrencyExchangeRate."Exchange Rate Amount") / 100;
                    ExchangeRateText := StrSubstNo(ExchangeRateTxt, CalculatedExchRate, CurrencyExchangeRate."Exchange Rate Amount");

                    IF HEader."Currency Factor" <> 0 THEN BEGIN
                        CurrencyFac := 1 / HEader."Currency Factor";
                        CurrencyFac := Round(CurrencyFac, 0.0001, '>');

                        LCYWithOutGST := HEader.Amount / HEader."Currency Factor";
                        LCYWithOutGST := Round(LCYWithOutGST, 0.00001, '>');

                        LCYWithGST := HEader."Amount Including VAT" / HEader."Currency Factor";
                        LCYWithGST := Round(LCYWithGST, 0.00001, '>');

                        LCYGST := (HEader."Amount Including VAT" - HEader.Amount) / HEader."Currency Factor";
                        LCYGST := Round(LCYGST, 0.0001, '>');

                        LCYDiscAmt := HEader."Invoice Discount Amount" / HEader."Currency Factor";
                ...
                TotalVATAmountText := VATAmountLine.VATAmountText;
                //VATPer := VATAmountLine."VAT %";
    ...
    trigger OnPreReport()
    begin
        if ReportOption = ReportOption::"COMMERCIAL INVOICE" then begin
            ProformaLbl := 'COMMERCIAL INVOICE'
        end else
            if ReportOption = ReportOption::"PROFORMA INVOICE" then begin
                ProformaLbl := 'PROFORMA INVOICE';
            end;
    ...
    var
        ...
        TotalOrdeAmount: Decimal;
        rSalesLine: Record "Sales line";
        TotalVATAmountText: text;
        ReportOption: Option "PROFORMA INVOICE","COMMERCIAL INVOICE";
        intSrNo: Integer;
        xItemNo: code[20];
